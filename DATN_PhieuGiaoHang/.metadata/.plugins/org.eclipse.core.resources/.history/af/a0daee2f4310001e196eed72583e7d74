...
//		model.addAttribute("order",deliveryNoteService.findByIds(id) );
	...
	// Cập nhật ngày 21/6/2023
	@RequestMapping({"/delivery-note/index"})
	public String admin() {
		return "pghNew/index";
	}
	// Cập nhật ngày 21/6/2023
	@RequestMapping("/delivery-note/create-new")
	public String deliveryNoteCreateNew(Model model, HttpServletRequest request) {
		model.addAttribute("orders",deliveryNoteService.getAll());
		return "assets/js/create";
	}
